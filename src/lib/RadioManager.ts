    ...
    constructor() {
        if (typeof window !== 'undefined') {
            ...
            this.audio.addEventListener('error', (e) => {
                console.error("Error encountered reproducing Audio", this.audio.error);
                if (this.mode === 'playlist' && this.audio.error && this.audio.error.code === 4 && this.isPlaying) {
                    console.log("Format error, skipping track in 2s...");
                    setTimeout(() => this.nextTrack(), 2000);
                } else if (this.mode === 'radio' && this.isPlaying) {
                    console.log("Broadcast failed, reconnecting in 5s...");
                    setTimeout(() => this.playUrl(`https://radio.deigowo.dev/stream.mp3?cb=${Date.now()}`), 5000);
                }
            })
        }
    }
    ...
    setMode(mode: RadioMode) {
        ...
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            if (mode === 'radio') {
                ...
                this.playUrl(`https://radio.deigowo.dev/stream.mp3?cb=${Date.now()}`);
            } else {
